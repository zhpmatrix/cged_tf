...
def do():
    ...
    train_x_char = tf.placeholder(tf.int64, [None,200],name='train_x_char')
    train_x_pos = tf.placeholder(tf.int64, [None,200],name="train_x_pos")
    train_x_word_old = tf.placeholder(tf.int64, [None,200],name="train_x_word_old")
    train_x_word_cur = tf.placeholder(tf.int64, [None,200],name="train_x_word_cur")
    train_y = tf.placeholder(tf.int64, [None,200],name="train_y")
    ...
    train_initializer = iterator.make_initializer(train_dataset, "train_initializer")
    ...
    y_predict = tf.cast(tf.argmax(y, axis=1), tf.int32, name='y_predict')
    ...
    if FLAGS.train:
        ...
        for epoch in range(FLAGS.epoch_num):
            ...
            sess.run(train_initializer, feed_dict={train_x_char:train_x_char_,train_x_pos:train_x_pos_,train_x_word_old:train_x_word_old_,train_x_word_cur:train_x_word_cur_,train_y:train_y_ })
            for step in range(int(train_steps)):
                smrs, loss, acc, gstep, _,pred = sess.run([summaries, cross_entropy, accuracy, global_step, train,y_predict],feed_dict={keep_prob: FLAGS.keep_prob})
        ...
        for step in range(int(test_steps)):
            ...
            for i in range(len(x_results)):
               ...
               for k,elem in enumerate(y_predict_text):
                   if elem[-1] == 'R':
                       x_text[k] = "\033[1;31;40m"+x_text[k]+"\033[0m"
                   if elem[-1] == 'M':
                       x_text[k] = "\033[1;30;44m"+x_text[k]+"\033[0m"
                   if elem[-1] == 'S':
                       x_text[k] = "\033[1;30;43m"+x_text[k]+"\033[0m"
                   if elem[-1] == 'W':
                       x_text[k] = "\033[1;30;45m"+x_text[k]+"\033[0m"
                   
               #print(''.join( x_text ), ''.join( y_predict_text ))
               while '<UNK>' in x_text:
                   x_text.remove('<UNK>')
               print(''.join( x_text ))
...
if __name__ == '__main__':
    ...
    parser.add_argument('--checkpoint_dir', help='checkpoint dir', default='ckpt_/model.ckpt', type=str)
    parser.add_argument('--summaries_dir', help='summaries dir', default='summaries_/', type=str)
